---
title: Gibson Typehammer 
heading: Gibson Typehammer keyboard
description: The perfect computer input device - ten fingers, ten keys
lead: >
  What if you could control the computer moving just your fingers while
  having your hands and arms otherwise at rest.
image:
...
The road to the ultimate keyboard ergonomics consists of many steps. The first one is saving the wrists from bending by splitting the keyboard into two independently positioned halves. I started this journey with the [Moonlanader](https://www.zsa.io/moonlander) by ZSA. It also incorporated step number two: linear columns of keys. The typewriter keys are - for mechanical reasons - arranged in linear rows that are offset relative to each other, but this forces rather awkward finger twisting. Linear columns allow for much more natural finger movement.
...
- Mouse, left thumb has the primary (left) mouse button (holding the control key down when clicking makes it the secondry (right) mouse button)
- Arrow keys, holding down the left thumb switches momentarily to the Page keys layer.
- Scroll wheel, both vertical and horizontal scrolling.
- Page keys: home, page up, page down, end.
